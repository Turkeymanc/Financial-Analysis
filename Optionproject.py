...
tickee = int(input("How many tickers do you want. Put a number:"  ))
for i in range(tickee):
    tickees = input("putt name of ticker: ")
    liquid_mid_caps.append(tickees)
...
umy["volatility"] = dmy
rr = np.array([])
for category, group in umy.groupby("symbol", sort=False):
    cool=(category + " - " + str(group["volatility"].mean()))
    oool= (group["volatility"].mean())
    rr = np.append(rr, oool)
...
inde=0
mars=(rr/arr)
for iphone in liquid_mid_caps:
    if mars[inde] > 1.5:
        mexico.append(iphone)
    
    print(iphone, "iv/hv ratio", mars[inde])
    inde+=1

for index in mexico:
    tim = chain(
        underlying_symbols=index,  
        expiration_date_gte="2026-09-10",
        expiration_date_lte="2026-09-15",
        strike_price_gte=price * 0.99,
        strike_price_lte=price * 1.01,
